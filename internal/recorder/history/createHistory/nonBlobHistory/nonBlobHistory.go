...
type NonBlobHistory struct{}

func NewNonBlobHistory() *NonBlobHistory {
	return &NonBlobHistory{}
}

func (n *NonBlobHistory) Create(path string, data any) error {
	// Create history directory for the file
	fileDir, err := n.getDirForFile(path)
	if err != nil {
		return err
	}

	// Generate the history file name
	title := n.getFileTitle(data)
...
func (n *NonBlobHistory) getDirForFile(path string) (string, error) {
...
func (n *NonBlobHistory) getFileTitle(data any) string {
