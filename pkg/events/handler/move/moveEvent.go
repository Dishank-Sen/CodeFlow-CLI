...
func (m *Move) Trigger() error {
	treePath := filepath.Join(".codeflow", "files", "fileTree.json")
	...
	codeflow := types.Rename{
		Path:       new,
		Name:       filepath.Base(new),
		Action:     "move",
		OldPath:    old,
	...
	if info != nil {
		codeflow.Size = info.Size()
	}

	if err := roottimeline.Save(codeflow); err != nil {
