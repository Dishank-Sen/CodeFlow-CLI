...
func Init() *cobra.Command {
	return &cobra.Command{
		Use:               "init",
		Short:             "initialize a new codeflowrepository",
		RunE:              initRunE,
...
func initPersistentPreRunE(cmd *cobra.Command, args []string) error {
	...
	if utils.CheckDirExist(rootDir) {
		log.Info(parentCtx, "Reinitializing codeflowrepository")
...
func initRunE(cmd *cobra.Command, args []string) error {
	...
	log.Info(ctx, "Initialized empty codeflowrepository")
	return nil
}
...
func reinitialize(ctx context.Context, cancel context.CancelFunc) error {
	...
	log.Info(ctx, "Reinitialized codeflowrepository")
	return nil
}
